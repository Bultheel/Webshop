...
contract("ChainList", function(accounts){
  ...
  it("should throw an exception if you try to buy an article that not exists", function(){
    return chainList.deployed().then(function(instance){
      chainListInstance= instance;
  ...
  it("should throw an exception if you try to buy your own article", function(){
    ...
    .catch(function(error){
      ...
      assert.equal(data[0].toNumber(), 1, "article is must be 1");
      assert.equal(data[1], seller, "seller must be " + seller);
      assert.equal(data[2],0x0,"buyer must be empty");
      assert.equal(data[3],articleName,"article name must be" + articleName);
      assert.equal(data[4],articleDescription,"article description must be " +articleDescription );
  ...
  //incorrect value
  it("should throw an exception if you try to buy your own article for a value different form its price", function(){
    return chainList.deployed().then(function(instance){
      chainListInstance = instance;
  ...
  //article has already been should
  it("should throw an exception if you try to buy your an article that has already been sold", function(){
    return chainList.deployed().then(function(instance){
      chainListInstance = instance;
      return chainListInstance.buyArticle(1,{from: buyer, value: web3.toWei(articlePrice , "ether")});
    }).then (function(){
      return chainListInstance.buyArticle(1,{from: web3.accounts[0], value: web3.toWei(articlePrice , "ether")});
    }).then(assert.fail)
    .catch(function(error){
      assert(true);
    }).then(function(){
